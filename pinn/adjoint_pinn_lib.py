...
def build_origin_block(
	...
	'''
	ONLY implement interconnect_method  = "Add"
		and tranfer_before_interconnect = False
	'''
...
def init_model_with_schemas(
	...
	input_record_schema = schema.Struct(
		('origin_input_1', schema.Scalar((np.float32, (sig_input_dim, )))), # sig
		('origin_input_2', schema.Scalar((np.float32, (tanh_input_dim, )))),  # tanh
	...
	output_record_schema = schema.Struct(
		...
		('adjoint_pred', schema.Scalar((np.float32, (
			sig_input_dim + tanh_input_dim, )))
		), # the concat of sig and tanh 
	)
