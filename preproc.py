...
def dc_iv_preproc(vg, vd, ids, scale, shift, 
    ...
    preproc_id = ids / scale['id'] * (np.exp(-slope * (vg + threshold)) + 1) if slope > 0 else ids/scale['id']

    def restore_func(vg, vd, ids):
        ori_vg = vg * scale['vg'] + shift
        ori_vd = vd * scale['vd']
        ori_id = ids * scale['id'] / (np.exp(-slope * (vg + threshold)) + 1) if slope > 0 else ids * scale['id']
